...
$sudo docker build --tag openosrs-$$ .
$sudo docker run --rm -d \
    ...
    --dns 8.8.8.8 \
    --mac-address "$(getmac)" \
    --mount type=bind,source="$PWD"/runelite,target=/home/openosrs/.runelite \
    --mount type=bind,source="$PWD"/jagexcache,target=/home/openosrs/jagexcache \
    openosrs-$$
